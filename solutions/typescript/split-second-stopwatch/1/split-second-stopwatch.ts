...
export class SplitSecondStopwatch {
    ...
    public get currentLap(): string {
        return SplitSecondStopwatch.formatTime(this._currentLap);
    }

    public get total(): string {
        return SplitSecondStopwatch.formatTime(this._total);
    }

    public get previousLaps(): readonly string[] {
        return [...this._previousLaps].map((lapTime: number) =>
            SplitSecondStopwatch.formatTime(lapTime),
        );
    }
    ...
    public advanceTime(duration: string): void {
        let seconds = SplitSecondStopwatch.parseTime(duration);
        switch (this.state) {
            case "running":
    ...
    private static parseTime(time: string): number {
        ...
        return SplitSecondStopwatch.secondsIn(
            dispHours,
            dispMinutes,
            dispSeconds,
        );
